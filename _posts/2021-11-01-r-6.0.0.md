...
Now we have all of the flights filtered that are the worst of the worst stored as a dbplyr lazy `tbl` with our DuckDB connection. This is an example of using Arrow -> DuckDB.
...
And just like that we've passed data back and forth between Arrow and DuckDB without having to write a signle file to disk!
