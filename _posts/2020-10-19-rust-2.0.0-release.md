...
Apache Arrow 2.0.0 is a significant release for the Apache Arrow project in general, and the Rust subproject in 
particular with almost 200 issues resolved by 15 contributors. In this blog post we will go through the main changes 
...
- Variable sized arrays (e.g. array of strings) have been internally refactored to more easily support their larger (64 
bit size offset) version. This allowed us to generalize some of the kernels to both (32 and 64) versions, and also 
...
- The Array trait now has get_buffer_memory_size and get_array_memory_size methods for determining the amount of memory 
allocated for the array.

# Parquet

Significant effort is underway to create a Parquet writer for Arrow data. This work has not been released as part of 
...
the parquet_derive for usage examples.
...
DataFusion now has a richer [DataFrame API][3] with improved documentation showing example usage, 
...
via SQL and the DataFrame API. UDFs now also have the same generality as DataFusion’s own functions, including variadic 
...
and batch updates. You can check out [this example][4] to learn how to declare and use a UDAF.
...
Arrow IPC is the format for serialization and interprocess communication. It is described in arrow.apache.org, and is 
...
Other ways to get involved include trying out Arrow on some of your own data and filing bug reports and helping to 
...
[3]: https://docs.rs/datafusion/2.0.0/datafusion/dataframe/trait.DataFrame.html
[4]: https://github.com/apache/arrow/blob/master/rust/datafusion/examples/simple_udaf.rs
