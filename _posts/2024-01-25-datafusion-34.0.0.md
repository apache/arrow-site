...
files, using `Parquet`, `CSV`, `JSON`, `ARROW` and user defined formats. [Benchmark results] show improvements up to 30% in some cases.

[Benchmark results]: https://github.com/apache/arrow-datafusion/pull/7655


Similarly to reading, data can be written to any [`ObjectStore`] implementation,
including AWS S3, Azure Blob Storage, GCP Cloud Storage, local files, and user
defined implementations. DataFusion has long supported reading from [hive style partitioned tables], and it 
now supports writing to such tables.
