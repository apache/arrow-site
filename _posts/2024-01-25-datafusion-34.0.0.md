...
reasonably good s experience out of the box as a [dataframe library] and
...
<figure style="text-align: center;">
  ...
  <figcaption>
    ...
    Note that several queries don't run on <code>25.0.0</code>, due to 
    unsupported SQL (Q9, Q11, Q12, Q14) or requiring too much memory (Q33).
  </figcaption>
</figure>

Here are some of the specific enhancements that contributed to this improvement:
...
partitioned tables] has long been supported, it is now possible to write such
tables.
...
For example, to write to a local file, you can use use the [`CREATE EXTERNAL TABLE` statement] for example:
...
DataFusion has begun appearing more in the wild. For example
* New projects built using DataFusion such as [lancedb], [GlareDB], and [Arroyo].
...
without sacrificing performance. We hope this paper helps people who may be
evaluating if DataFusion is a good fit for their needs.
...
1. *Modularity*: Make Datafusion even more modular, such as [unifying how
   built in and user functions are defined], making it easier to customize 
...
[unifying how built in and user functions are defined]: https://github.com/apache/arrow-datafusion/issues/8045
