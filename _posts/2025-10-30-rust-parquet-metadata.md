---
layout: post
title: "Speeding up Parquet Metadata Parsing in Rust with a custom Thrift Parser"
date: "2025-10-08 00:00:00"
author: "Andrew Lamb (InfluxData) and Ed Seidl (LLNL)"
categories: [release]
---
<!--
{% comment %}
...
Parquet files store metadata using the [Thrift compact binary encoding format]
and the Thrift definitions for Parquet metadata can be found in
[parquet.thrift]. This encoding uses a variable length encoding, meaning that
the length of each element is written first, and then the element itself.
Primitive types such as integers similarly use a variable length encoding where
smaller values use fewer bytes. Strings and lists are prefixed with their length
in bytes. This encoding makes the format space efficient, but its variable
length nature requires scanning all previous fields to find any particular field.
...
*Figure X:* Parquet metadata serialized using the [Thrift compact binary encoding format]. Each field is
stored using a variable number of bytes. Primitive types use a variable length encoding and
strings and lists are prefixed with their length in bytes. 
...
Parsing Parquet metadata is the process of decoding the thrift binary into
in-memory structures that can be used for computation. For example the encoded
. 
Given the design point for network message passing, the The code generated by 
thrift compilers typically parse all fields in a single pass over the binary data,
copying data into in-memory structures as they go. 
