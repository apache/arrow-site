...
1. The generated is not easy to modify (it must be re-generated from from the thrift definitions when the definitions change)
2. The generated code must be general purpose, and easy to embed, and typically has
...
3. The thrift compilers are quite stable, which is important to enable embedding 
...
And here is the corresponding Rust structure annotated with the custom thrift parsing macros:
