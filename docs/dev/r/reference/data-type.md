...
always yields a
[`bit64::integer64`](https://bit64.r-lib.org/reference/bit64-package.html)
object) by setting `options(arrow.int64_downcast = FALSE)`. For values
nested inside a list type, this decision is made once for all values in
the column, so every element of the list uses the same R type.
