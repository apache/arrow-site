...
- x:
  ...
  objects whose datasets should be combined. If this argument is
  specified it will be used to construct a `UnionDatasetFactory` and
...
- format:
  ...
  Default is "parquet", unless a `delimiter` is also specified, in which
  case it is assumed to be "text".
...
- partitioning:
  ...
  - A `Schema`, in which case the file paths relative to `sources` will
    ...
    For example, `schema(year = int16(), month = int8())` would create
  ...
  - A `HivePartitioning` or `HivePartitioningFactory`, as returned by
    [`hive_partition()`](https://arrow.apache.org/docs/r/reference/hive_partition.md)
    which parses explicit or autodetected fields from Hive-style path
    segments
...
- hive_style:
  ...
  Logical: if `partitioning` is a character vector or a `Schema`, should
  it be interpreted as specifying Hive-style partitioning? Default is
  `NA`, which means to inspect the file paths for Hive-style
...
- ...:
  ...
  [`FileFormat$create()`](https://arrow.apache.org/docs/r/reference/FileFormat.md).
  For CSV options, note that you can specify them either with the Arrow
  C++ library naming ("delimiter", "quoting", etc.) or the `readr`-style
  naming used in
  [`read_csv_arrow()`](https://arrow.apache.org/docs/r/reference/read_delim_arrow.md)
  ("delim", "quote", etc.). Not all `readr` options are currently
  supported; please file an issue if you encounter one that `arrow`
  should support.

## Value

A `DatasetFactory` object. Pass this to
[`open_dataset()`](https://arrow.apache.org/docs/r/reference/open_dataset.md),
in a list potentially with other `DatasetFactory` objects, to create a
`Dataset`.
...
[`open_dataset()`](https://arrow.apache.org/docs/r/reference/open_dataset.md)
directly. Use `dataset_factory()` when you want to combine different
directories, file systems, or file formats.
