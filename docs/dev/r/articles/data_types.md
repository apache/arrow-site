...
[`dplyr::glimpse()`](https://pillar.r-lib.org/reference/glimpse.html) to
...
- `name` is labelled `<chr>` (character vector) in the data frame; it is
  labelled `<string>` (a string type, also referred to as utf8 type) in
  the Arrow Table
...
first check to see if a Schema has been provided – see
...
to create Arrow data from R objects and
[`as.vector()`](https://rdrr.io/r/base/vector.html) to create R data
...
|-----------------|---------------------------------------------------------------------:|---------------------:|---------------------:|
| 8 bit unsigned  |  [`uint8()`](https://arrow.apache.org/docs/r/reference/data-type.md) |                    0 |                  255 |
| 16 bit unsigned | [`uint16()`](https://arrow.apache.org/docs/r/reference/data-type.md) |                    0 |                65535 |
| 32 bit unsigned | [`uint32()`](https://arrow.apache.org/docs/r/reference/data-type.md) |                    0 |           4294967295 |
| 64 bit unsigned | [`uint64()`](https://arrow.apache.org/docs/r/reference/data-type.md) |                    0 | 18446744073709551615 |
| 8 bit signed    |   [`int8()`](https://arrow.apache.org/docs/r/reference/data-type.md) |                 -128 |                  127 |
| 16 bit signed   |  [`int16()`](https://arrow.apache.org/docs/r/reference/data-type.md) |               -32768 |                32767 |
| 32 bit signed   |  [`int32()`](https://arrow.apache.org/docs/r/reference/data-type.md) |          -2147483648 |           2147483647 |
| 64 bit signed   |  [`int64()`](https://arrow.apache.org/docs/r/reference/data-type.md) | -9223372036854775808 |  9223372036854775807 |
...
data types, in which numeric values are specified in decimal format
rather than binary. Decimals in Arrow come in two varieties, a 128 bit
version and a 256 bit version, but in most cases users should be able to
use the more general
[`decimal()`](https://arrow.apache.org/docs/r/reference/data-type.md)
data type function rather than the specific
...
- `scale` specifies the number of digits that should be stored after the
  decimal point. If you set `scale = 2`, exactly two digits will be
  stored after the decimal point. If you set `scale = 0`, values will be
  rounded to the nearest whole number. Negative scales are also
  permitted (handy when dealing with extremely large numbers), so
...
objects possess a
[`cast()`](https://arrow.apache.org/docs/r/reference/cast.md) method:
...
[`utf8()`](https://arrow.apache.org/docs/r/reference/data-type.md) and
[`large_utf8()`](https://arrow.apache.org/docs/r/reference/data-type.md)
data type functions. The distinction between these two Arrow types is
unlikely to be important for R users, though the difference is discussed
in the article on [data object
...
C++; however, only
[`float64()`](https://arrow.apache.org/docs/r/reference/data-type.md) is
used in arrow as the function
[`double()`](https://rdrr.io/r/base/double.html) already exists in base
R
