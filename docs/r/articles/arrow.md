...
- [`arrow_table()`](https://arrow.apache.org/docs/r/reference/table.md)
  allows you to create Arrow tables without directly using the `Table`
  object
- [`read_parquet()`](https://arrow.apache.org/docs/r/reference/read_parquet.md)
  allows you to open Parquet files without directly using the
...
analogous to data frames and have similar behavior. The
[`arrow_table()`](https://arrow.apache.org/docs/r/reference/table.md)
function allows you to generate new Arrow Tables in much the same way
that [`data.frame()`](https://rdrr.io/r/base/data.frame.html) is used to
...
native R data frames (or tibbles) using
...
package and write it to a Parquet file using
...
We can then use
[`read_parquet()`](https://arrow.apache.org/docs/r/reference/read_parquet.md)
to load the data from this file. As shown below, the default behavior is
to return a data frame (`sw_frame`) but when we set
`as_data_frame = FALSE` the data are read as an Arrow Table
(`sw_table`):
...
[`read_parquet()`](https://arrow.apache.org/docs/r/reference/read_parquet.md)
but is often more convenient – especially for very large data sets – to
scan the folder and “connect” to the data set without loading it into
memory. We can do this using
...
The example below illustrates how you can use
[`s3_bucket()`](https://arrow.apache.org/docs/r/reference/s3_bucket.md)
to refer to a an S3 bucket, and use
[`open_dataset()`](https://arrow.apache.org/docs/r/reference/open_dataset.md)
to connect to the data set stored there:
...
function in reticulate as shown below:
