...
- chunk_size:
  ...
  corresponds to how many rows will be in each row group in parquet. If
  `NULL`, a best guess will be made for optimal size (based on the
...
- version:
  ...
  parquet version: "1.0", "2.4" (default), "2.6", or "latest" (currently
  equivalent to 2.6). Numeric values are coerced to character.
...
- data_page_size:
  ...
  within a column chunk (in bytes). Default 1 MiB.
...
disable compression, set `compression = "uncompressed"`. Note that
"uncompressed" columns may still have dictionary encoding.
