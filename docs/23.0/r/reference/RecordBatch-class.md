...
- `$SelectColumns(indices)`: Return a new record batch with a selection
  of columns, expressed as 0-based integers.
...
- `$Filter(i, keep_na = TRUE)`: return an `RecordBatch` with rows at
  positions where logical vector (or Arrow boolean Array) `i` is `TRUE`.

- `$SortIndices(names, descending = FALSE)`: return an `Array` of
  integer row positions that can be used to rearrange the `RecordBatch`
  in ascending or descending order by the first named column, breaking
  ties with further named columns. `descending` can be a logical vector
  of length one or of the same length as `names`.
